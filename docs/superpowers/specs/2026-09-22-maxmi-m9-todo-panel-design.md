...
- Migrations head is v13; `MemoryDataControls.pruneMemory/deleteAllMemory` already cover `action_items`.

## 3. Components

### 3a. `OptionDoubleTapMonitor` (Sources/MaxMi)
- `NSEvent.addGlobalMonitorForEvents(matching: [.flagsChanged, .keyDown])` plus the local-monitor twin so taps register while MaxMi's own windows are frontmost. Requires the Accessibility grant MaxMi already holds; no Input Monitoring, no CGEventTap.
- Pure state machine `OptionDoubleTapDetector` (Sources/MaxMiActivity so it is unit-testable): input events `optionDown(t)`, `optionUp(t)`, `otherKeyOrModifier(t)`; output `fired`. A *tap* is optionDown→optionUp with no `otherKeyOrModifier` in between and with hold ≤ 400 ms. Two taps whose downs are ≤ 350 ms apart fire once; the detector then resets. Any `otherKeyOrModifier` resets. Constants are `static let`s.
...
- Migration **v14**: `ALTER TABLE action_items ADD COLUMN remind_at_ms INTEGER NULL; ADD COLUMN reminded_at_ms INTEGER NULL;` index on `(status, remind_at_ms)`. `DatabaseRecovery` derives the migration set from the migrator (no hand-edited lists). `ActionItem` gains `remindAtMs: EpochMs?`, `remindedAtMs: EpochMs?`.
...
- Prune/delete-all need no change (rows live in `action_items`); a test asserts the new columns are gone with the row.

### 3e. Hourly review `remind_at` (Sources/MaxMiActivity)
- Review operation gains optional `remind_at: String?` (ISO-8601 with offset). The prompt adds one instruction: set `remind_at` only when the evidence states a concrete time or deadline for the item; otherwise omit it. Nothing else in the prompt changes; fencing and validation stay as they are.
...
- Items originate only from captures the shared `SourceCloudEligibility` policy allowed; the panel adds no new read path over raw content (it reads `action_items` and the check-in summary only).

## 5. Testing (XCTest only)
- `OptionDoubleTapDetectorTests`: two fast taps fire once; second down at 351 ms does not; hold > 400 ms is not a tap; option+key chord resets; three taps fire once then reset.
- `TodoPanelViewModelTests` (MaxMiUITests, fakes): load orders newest first and caps at 25; done/dismiss call the repository and remove the row; keyboard selection wraps correctly; empty state; check-in header line.
...
- `HourlyAgentTests`: op with valid `remind_at` sets the reminder; invalid drops the field but keeps the item; goldens updated for the prompt line.
