...
Sources/MaxMiStore/Migrations.swift        MODIFY: v4 — activity_app_visits, activity_sessions,
                                                        activity_session_evidence, agent_runs,
                                                        agent_action_items, agent_action_item_events
Sources/MaxMiStore/MaxMiDatabase.swift      MODIFY: enable PRAGMA foreign_keys = ON (Codex)
Sources/MaxMi/ActivityPrivacySheet.swift    NEW (M6a): SwiftUI consent + global/per-app exclusion sheet
...
-- Immutable per-session EVIDENCE snapshot (▲REV2 — Codex critical): a `versions` row is
-- overwritten throughout its hour, so pointing at version_id alone can lose/misattribute the
-- earlier session's content when an app is revisited within the hour. So we snapshot the
...
**Session identity & staleness (▲REV):** sessions are identified by their own UUID (never by app+url), so revisiting a page next week creates a NEW session. `source_hash` = hash over the session's `activity_session_evidence` snapshots; a session is (re)summarized only when it's closed/idle AND its `source_hash` changed — never while a current-hour version is still mutating. Summaries + "expand to detail" read the **immutable evidence snapshots**, not the mutable `versions` rows (▲REV2), so a same-hour revisit can't corrupt an earlier session's content.

**FK enforcement (▲REV2 — Codex):** `MaxMiDatabase` must enable `PRAGMA foreign_keys = ON` in its GRDB connection setup for the `REFERENCES` constraints above to actually enforce; the v4 migration/task adds this (it's currently off).
...
- **Activity privacy controls in M6a** (▲REV2 — with a concrete surface + semantics): a lightweight **M6a privacy sheet** (a menu command "Activity Privacy…" opening a small SwiftUI sheet — NOT the full M6c Settings window) shown before synthesis first runs. It offers a **global activity-synthesis toggle** + **per-app exclusion** (`activity.disabled_app_bundle_ids`). Semantics: an excluded/globally-disabled app produces **NO visits, NO sessions, NO evidence snapshots** (not merely "no Gemini summary") — so the cleartext SQLite timeline never reveals excluded app use. Toggling an app to excluded **deletes that app's existing activity rows** (visits/sessions/evidence). Sensitive-denylist apps are always excluded regardless.
...
4. Derived text (summaries, action items, evidence, resolution_evidence) encrypted at rest; **per-app exclusion + global toggle work in M6a** (excluded apps produce no visits/sessions/evidence; toggling deletes existing rows); Gemini-consent disclosure shown before first synthesis.
5. Full Settings window (Launch at Login, pause, per-app disable UI, Check for Updates, model/key status) — M6c.
6. UI is clean/animated/smooth (spring insert/resolve, measured-60fps target, vibrancy, **intentional always-dark branded palette via Theme tokens** — not auto light/dark) — the ship-readiness bar.
